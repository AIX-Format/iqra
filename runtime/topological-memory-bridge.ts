...
export class TopologicalMemoryBridge {
  ...
  /**
   * High-level context retrieval for the Gateway | استرجاع السياق للبوابة
   */
  public async retrieveContext(input: string): Promise<any> {
    return this.retrieve(MemoryType.SEMANTIC, input, ['context']);
  }

  /**
   * Persist a full cognitive cycle | حفظ دورة المعرفة الكاملة
   */
  public async persistCycle(sessionId: string, cycleData: any): Promise<void> {
    await this.save(MemoryType.EPISODIC, `${sessionId}_${Date.now()}`, cycleData, ['cycle', 'sovereign']);
  }
